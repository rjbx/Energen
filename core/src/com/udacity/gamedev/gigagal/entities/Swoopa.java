...
public class Swoopa implements DestructibleHazard {
    ...
    // ctor
    public Swoopa(Box box, Level level) {
        this.box = box;
        this.level = level;
        position = new Vector2(box.getRight() + Constants.SWOOPA_CENTER.x * 3, box.getTop() + Constants.SWOOPA_CENTER.y * 3);
    ...
    public void update(float delta) {
        ...
        && Utils.secondsSince(descentStartTime) > 2.75f) {
            if (position.y > (box.getTop() + Constants.SWOOPA_COLLISION_HEIGHT)) {
        ...
        if (position.x < (camera.x - worldSpan.x)) {
            ...
            position.x = box.getRight() + (Constants.WORLD_SIZE) / 1.5f;
            position.y = (box.getTop() + (Constants.WORLD_SIZE));
