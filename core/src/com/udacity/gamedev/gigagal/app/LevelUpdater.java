        ...
        if (ground instanceof Portable) {
            if (!((Portable) ground).isBeingCarried() && Helpers.overlapsPhysicalObject(gigaGal, ground)) {
                ...
                if (InputControls.getInstance().leftButtonPressed || InputControls.getInstance().rightButtonPressed) {
                    ((Portable) ground).toss((gigaGal.getVelocity().x + Helpers.absoluteToDirectionalValue(ground.getWidth() * 13, gigaGal.getDirectionX(), Enums.Orientation.X)) * ((Portable) ground).weightFactor());
                }
            }
        }
        if (ground instanceof Nonstatic) {
