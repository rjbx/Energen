...
public class Slick implements Skateable, Ground {
    ...
    public void render(SpriteBatch batch, Viewport viewport) {
        Helpers.drawTextureRegion(batch, viewport, ImageLoader.getInstance().getGroundAssets().slick.getKeyFrame(Helpers.secondsSince(startTime), true), position, adjustedCenter, scale);
    }

    @Override public final Vector2 getPosition() { return position; }
    @Override public final float getHeight() { return Constants.SLICK_CENTER.x * 2 * scale.x; }
    @Override public final float getWidth() { return Constants.SLICK_CENTER.y * 2 * scale.y; }
